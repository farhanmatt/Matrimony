...
export default async function AdminUsersPage({
  ...
  const userColumnWidths: Record<UserColumnKey, string> = {
    user: "22%",
    details: "28%",
    profile: "18%",
    status: "12%",
  ...
  const getColumnWidth = (key: UserColumnKey) =>
    visibleColumns.length === 1 ? "100%" : userColumnWidths[key];
                ...
                {users.map((user) => {
                  ...
                  return (
                    <tr key={row.id} className="transition-colors hover:bg-slate-50/80">
                      {visibleColumns.map((column) => (
                        <td key={`${row.id}-${column.key}`} className="px-4 py-4 align-middle">
                          {column.render(row)}
                        </td>
                      ))}
                    </tr>
