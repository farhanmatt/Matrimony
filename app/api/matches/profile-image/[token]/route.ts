...
import { findMatch, findUnlockForProfiles } from "@/lib/utils/matching";
...
function buildFallbackPreviewSvg() {
  return `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 480 560" role="img" aria-label="Protected mutual interest profile preview">
      <defs>
...
function buildFallbackPreviewResponse() {
  return new NextResponse(buildFallbackPreviewSvg(), {
    headers: {
      ...
      "Content-Disposition": 'inline; filename="match-profile-preview.svg"',
    },
  });
}
...
async function loadPreviewSourceBuffer(source: string) {
  ...
  if (!safeSource || !safeSource.startsWith("https://")) {
    throw new Error("Unsupported match profile preview source");
  }

  const response = await fetch(safeSource, { cache: "no-store" });

  if (!response.ok) {
    throw new Error(`Failed to fetch match preview source: ${response.status}`);
  }
...
function buildImageResponse(imageBuffer: Buffer) {
  return new NextResponse(new Uint8Array(imageBuffer), {
    headers: {
      ...
      "X-Robots-Tag": "noindex, noimageindex, noarchive",
      "Cross-Origin-Resource-Policy": "same-origin",
      "Content-Disposition": 'inline; filename="match-profile.jpg"',
    },
  });
}
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const match = await findMatch(viewerProfile.id, targetProfile.id);
  if (!match) {
  ...
  try {
    ...
    console.error("Failed to render protected match profile preview:", error);
