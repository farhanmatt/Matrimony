...
export default function LockedProfileCard({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-rose-100 dark:border-slate-800 overflow-hidden">
      {/* Blurred photo with watermark */}
      <div className="relative h-56 bg-gradient-to-br from-rose-50 to-pink-100 dark:from-slate-800 dark:to-slate-800 profile-locked">
        {primaryPhoto ? (
          <Image
            src={primaryPhoto}
          ...
          <div className="flex items-center justify-center h-full not-profile-locked">
            <div className="w-20 h-20 bg-rose-200 dark:bg-slate-800 rounded-full flex items-center justify-center">
              <span className="text-3xl font-display font-bold text-rose-300 dark:text-slate-600">?</span>
            </div>
          </div>
        )}
        {/* Watermark overlay */}
      ...
      <div className="p-5">
        <h3 className="font-display font-bold text-gray-900 dark:text-slate-100 text-lg mb-1">
          {profile.fullName.split(" ")[0]} {/* First name only */}
          <span className="text-gray-300 dark:text-slate-600 font-normal"> ••••</span>
        </h3>

        <div className="space-y-1 mb-4 text-sm text-gray-500 dark:text-slate-400">
        ...
        {/* Pricing */}
        <div className="bg-rose-50 dark:bg-slate-800 rounded-xl p-3 mb-4 text-xs text-gray-700 dark:text-slate-300">
          ...
          <div className="flex justify-between mb-1">
            ...
            <span>₹{profileUnlockAmount}</span>
          </div>
          <div className="flex justify-between mb-1">
            <span>Per Profile Chat Amount</span>
            <span>₹{perProfileChatAmount}</span>
          </div>
          <div className="flex justify-between font-bold text-rose-600 dark:text-rose-400 border-t border-rose-200 dark:border-slate-700 pt-1 mt-1">
            <span>Total</span>
            <span>₹{total}</span>
          </div>
        </div>
