...
const profilePresenceOptions = [
  { value: "", label: "Any Profile" },
  { value: "with", label: "With Profile" },
  { value: "without", label: "Without Profile" },
] as const;
...
export default function AdminUserFilters({ activeCount = 0 }: AdminUserFiltersProps) {
  ...
  const currentActiveCount = useMemo(() => {
    const keys: (keyof DraftFilters)[] = ["profileStatus", "hasProfile", "joinedFrom", "joinedTo"];
          ...
          <div className="ui-modal-pop relative z-10 w-full max-w-3xl rounded-3xl border border-gray-100 bg-white p-6 shadow-2xl sm:p-7">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 font-display text-2xl font-bold text-gray-900">
                  Narrow your user list
                </h2>
                <p className="mt-1 text-sm text-gray-500">
                  Use user filters to find the right records faster.
                </p>
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <select
                  value={drafts.profileStatus}
                  onChange={(event) =>
                    setDrafts((current) => ({ ...current, profileStatus: event.target.value }))
                  }
              ...
              <div>
                ...
                <select
                  value={drafts.hasProfile}
                  onChange={(event) =>
                    setDrafts((current) => ({ ...current, hasProfile: event.target.value }))
                  }
                  className="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500"
                >
                  {profilePresenceOptions.map((option) => (
                    <option key={option.value || option.label} value={option.value}>
                      {option.label}
                    </option>
                  ))}
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-700">Joined From</label>
                <input
                  type="date"
                  value={drafts.joinedFrom}
                  onChange={(event) =>
                    setDrafts((current) => ({ ...current, joinedFrom: event.target.value }))
                  }
                  className="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500"
                />
              </div>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-700">Joined To</label>
                <input
                  type="date"
                  value={drafts.joinedTo}
                  onChange={(event) =>
                    setDrafts((current) => ({ ...current, joinedTo: event.target.value }))
                  }
                  className="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500"
                />
              </div>
