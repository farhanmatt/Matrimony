...
function shouldSkipTrackedFetch(
  input: Parameters<typeof window.fetch>[0],
  init?: Parameters<typeof window.fetch>[1]
) {
  const autoRefreshState = window as typeof window & {
    __dashboardAutoRefreshCount?: number;
  };

  if ((autoRefreshState.__dashboardAutoRefreshCount ?? 0) > 0) {
    return true;
  }
  ...
  return requestHeaders?.get("x-skip-loading") === "1";
}
...
export default function AppLoadingProvider({
  ...
  useEffect(() => {
    ...
    const trackedFetch: typeof window.fetch = async (...args) => {
      const skipTrackedLoading = shouldSkipTrackedFetch(args[0], args[1]);

      if (!skipTrackedLoading) {
        startLoading();
      }
