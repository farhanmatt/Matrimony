...
export default function LikedProfilePreviewCard({
  ...
  useEffect(() => {
    setShortlisted(readShortlistedProfileIds().includes(profile.id));
  }, [profile.id]);
  ...
  const handleUnlike = async () => {
    ...
    try {
      ...
      if (shortlisted) {
        setShortlistedProfileId(profile.id, false);
  ...
  const handleShortlistToggle = (event: ReactMouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      setShortlistedProfileId(profile.id, nextShortlisted);
